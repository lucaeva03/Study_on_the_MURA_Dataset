...
# --------------------- VALUTAZIONE MODELLO DenseNet169 --------------------------- #
...
y_prob = knn.predict_proba(X_valid)[:, 1]  # Probabilità per la classe positiva (1)
fpr, tpr, thresholds = roc_curve(y_valid, y_prob)
roc_auc = auc(fpr, tpr)

# Salva le coordinate ROC in un file CSV
roc_data = list(zip(fpr, tpr))
with open('Z_roc_coordinates_KNN.csv', 'w', newline='') as file:
...
plt.plot([0, 1], [0, 1], color='gray', linestyle='--', lw=1.5)  # Linea diagonale di riferimento

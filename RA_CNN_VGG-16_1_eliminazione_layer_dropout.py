...
# --------------------------------- CARICAMENTO MODELLO CNN VGG-16 ----------------------------------- #

print(f"\n\nCARICAMENTO MODELLO CNN VGG-16\n ------------------------------------")

# Carica il modello VGG-16 preaddestrato senza i livelli superiori (include_top=False)
...
# Costruzione del modello senza Dropout
x = base_model.output
x = Flatten()(x)
x = Dense(512, activation='relu')(x)  # Rimosso il livello Dropout
output_layer = Dense(2, activation='softmax')(x)

# ------------------------- DEFINIZIONE e ADDESTRAMENTO DEL MODELLO CNN VGG-16 ----------------------------- #
...
# Converti i dati in formato adatto per il modello VGG-16 (3 canali)
X_train_small_vgg = np.repeat(X_train_small.reshape(-1, 64, 64, 1), 3, axis=-1)
X_valid_vgg = np.repeat(X_valid.reshape(-1, 64, 64, 1), 3, axis=-1)

# Addestra il modello
model.fit(X_train_small_vgg, y_train_small, epochs=10, batch_size=32, validation_data=(X_valid_vgg, y_valid))

# Predizioni del modello
y_pred = model.predict(X_valid_vgg)
y_pred_classes = y_pred.argmax(axis=1)

# --------------------- VALUTAZIONE MODELLO CNN VGG-16 --------------------------- #

print("\n\nVALUTAZIONE MODELLO CNN VGG-16 --------------------------------------------")

# Valutazione del modello
loss, accuracy = model.evaluate(X_valid_vgg, y_valid)

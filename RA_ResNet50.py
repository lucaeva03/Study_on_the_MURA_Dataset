...
# Carica il modello ResNet50 preaddestrato senza i livelli superiori (include_top=False)
...
# ------------- DEFINIZIONE e ADDESTRAMENTO DEL MODELLO CNN ResNet50 ----------------------------- #
...
# Converti i dati in formato adatto per il modello ResNet50 (3 canali)
X_train_small_resnet = np.repeat(X_train_small.reshape(-1, 64, 64, 1), 3, axis=-1)
X_valid_resnet = np.repeat(X_valid.reshape(-1, 64, 64, 1), 3, axis=-1)

# Addestra il modello
model.fit(X_train_small_resnet, y_train_small, epochs=10, batch_size=32, validation_data=(X_valid_resnet, y_valid))

# Predizioni del modello
y_pred = model.predict(X_valid_resnet)
y_pred_classes = y_pred.argmax(axis=1)

# --------------------- VALUTAZIONE MODELLO CNN RESNET50 --------------------------- #

print("\n\nVALUTAZIONE MODELLO CNN RESNET50 --------------------------------------------")

# Valutazione del modello
loss, accuracy = model.evaluate(X_valid_resnet, y_valid)
